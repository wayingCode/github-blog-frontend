<template>
  <div>
    <section
      class="page-header"
      :style="'background-image: linear-gradient(120deg, ' + metadata.backgroundColorLeft +', ' + metadata.backgroundColorRight + ');color: ' +metadata.fontColor +';'"
    >
      ...
      <h1 class="project-name">{{ metadata.blogTitle }}</h1>
      <h2 class="project-tagline">{{ metadata.blogDescribe }}</h2>
      <a
        ...
        <el-row>
          <el-col :span="10">
            ...
            >
              ...
              <el-submenu index="#webSites" v-if="metadata.webSites.length > 0">
                <template slot="title">其他网站</template>
                <el-menu-item
                  :index="'#webSites-' + index"
                  v-for="(item, index) in metadata.webSites"
                  :key="'#webSites' + index"
          ...
          >
            ...
            <audio ref="music" loop autoplay v-if="metadata.audioAutoPlay">
          ...
          <el-col :span="4" style="text-align: right">
            <div style="font-size: 20px; color: #606266; margin-top: 5px;margin-bottom:10px">
              <b>{{ metadata.githubUsername }}</b>
            </div>
          ...
          <el-col :span="2" style="text-align: center">
            ...
            <el-popover
              ref="bigAvatar"
              placement="top-start"
              :title="metadata.githubUsername"
              width="200"
              trigger="hover"
            >
              <i class="el-icon-star-on"></i>&emsp;{{ metadata.githubUsername }}
              <br />
...
export default {
  ...
  methods: {
    selectTopbar(index) {
      ...
      switch (index) {
        case "#githubHome":
          window.open("https://github.com/" + this.metadata.githubUsername);
          break;
        case "#blog":
          if (this.blog) {
            window.open(
